...
def classify_hand_shape(hand_landmarks):
    ...
    thumb_open = is_finger_open(4, 3, 2, 0)
    index_open = is_finger_open(8, 6, 5, 0)
    middle_open = is_finger_open(12, 10, 9, 0)
    ring_open = is_finger_open(16, 14, 13, 0)
    pinky_open = is_finger_open(20, 18, 17, 0)
...
def rsp_game(cap):
    ...
    def update_frame():
        ...
        if game_state == "준비":
            ...
            if countdown > 0:
                image = put_text(image, f"안 내면 진다, 가위 바위 보! {countdown}", (50, 50), 30, (255, 0, 0))
            else:
                game_state = "결과"
                result = determine_winner(current_hand_shape, computer_choice)
                last_game_time = current_time
            ...
            image = put_text(image, f"플레이어: {current_hand_shape}", (50, 50), 30, (255, 0, 0))
            image = put_text(image, f"컴퓨터: {computer_choice}", (50, 100), 30, (255, 0, 0))
            image = put_text(image, f"결과: {result}", (50, 150), 30, (255, 0, 0))
        ...
        if game_state == "준비":
            image = put_text(image, "가위바위보 준비!", (50, 50), 30, (255, 0, 0))
